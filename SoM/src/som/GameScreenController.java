...
public class GameScreenController implements Initializable {
    ...
    public final static int CURRENT_PLAYER_IS_CPU = 1;
    public final static int CURRENT_PLAYER_IS_HUMAN = 0;
 ...
 /*
    ...
    public void startGame() {
        ...
        fillPlayerInfo();
        if (currentPlayer.isCpu(currentPlayer)) {
            desactivateEvent();
        }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                while (true) {
                    if (playerState == CURRENT_PLAYER_IS_CPU) {
                        HexVertex vert = null;
                        setSelectedItem(null);
                        setSelectedItem(ai.verTexChoiceTobuildSettlement(board));
                        while (!canBuildSettlement((HexVertex) selectedItem, currentPlayer)) {
                            setSelectedItem(ai.verTexChoiceTobuildSettlement(board));
                            vert = (HexVertex) selectedItem;
                        }
                            try {
                                System.out.println("inside try :" + currentPlayer.isCpu(currentPlayer));
                                if (gameState == PLACING_SETTLEMENT || gameState == PLACING_FREE_SETTLEMENT) {
                                    buildSettlement();
                                    System.out.println("player state :" + currentPlayer.isCpu(currentPlayer));
                                }
                        ...
                        if ((((HexVertex)selectedItem).getAsset() != null) && ((((HexVertex)selectedItem).isOwned()))&&((((HexVertex)selectedItem).getAsset().getPlayer().equals(currentPlayer)))) {
                            System.out.println("the vert belong to : "+((HexVertex)selectedItem).getAsset().getPlayer().getNickname());
                            HexVertex v=((HexVertex)selectedItem);
                            int i=0;
                            setSelectedItem(ai.EdgeChoiceForBuildRoad(board,((HexVertex)selectedItem),((Cpu)currentPlayer),i));  
                            HexEdge hexEdge = (HexEdge)selectedItem;
                            while (!(canBuildRoad(hexEdge, currentPlayer))) {
                                i++;
                                setSelectedItem(ai.EdgeChoiceForBuildRoad(board,v,((Cpu)currentPlayer),i));  
                                hexEdge = (HexEdge)selectedItem;
                            }
                    ...
                    //System.out.println("player state is : "+ playerState);
                }
            }
        }).start();
    ...
    public void setUIFromGameState() {
        if (playerState == CURRENT_PLAYER_IS_HUMAN) {
            switch (gameState) {
                case NEW_GAME:
    ...
    public void createTestPlayers() {
        ...
        Cpu dek = new Cpu("Dehkhoda", new int[]{2, 2, 2, 2, 2}, Color.MEDIUMSEAGREEN, board);
    ...
    public void desactivateEvent() {
        for (Hex h : board.transHexList) {
            h.removeEventHandler(MouseEvent.MOUSE_CLICKED, e -> {

            });
        }
        for (Hex hex : board.hexList) {
            hex.removeEventHandler(MouseEvent.MOUSE_CLICKED, e -> {

            });
        }
        for (HexVertex hexVertex : board.vertexList) {
            hexVertex.removeEventHandler(MouseEvent.MOUSE_CLICKED, e -> {
            });

        }

        for (HexEdge hexEdge : board.edgeList) {
            hexEdge.removeEventHandler(MouseEvent.MOUSE_CLICKED, e -> {
    ...
    public void activateEvent() {
        ...
        for (Hex hex : board.hexList) {
            hex.addEventHandler(MouseEvent.MOUSE_CLICKED, e -> {
                ...
                System.out.println( // hex.getTokenValue()+"\n"
                        // +hex.getVerticies()+"\n+---------+\n"
                        );
                int vertexCounter = 0;
        ...
        for (HexVertex hexVertex : board.vertexList) {
            hexVertex.addEventHandler(MouseEvent.MOUSE_CLICKED, e -> {
                ...
                try {
                    if (gameState == PLACING_SETTLEMENT || gameState == PLACING_FREE_SETTLEMENT) {
                        buildSettlement();
                        System.out.println(currentPlayer.isCpu(currentPlayer));
                    }
                } catch (IOException e1) {
                    e1.printStackTrace();
                }
    ...
    public synchronized void setUpPlayerState() {
        if (currentPlayer.isCpu(currentPlayer)) {
            playerState = 1;
        } else {
            playerState = 0;
