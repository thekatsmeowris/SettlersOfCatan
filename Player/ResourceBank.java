...
public class ResourceBank {
	...
	public ResourceBank(){		
		// Edited
		AResource soy = new AResource(MAX);    //creating objects from AResource class
		AResource hemp = new AResource(MAX);
		AResource plastic = new AResource(MAX);
		AResource glass = new AResource(MAX);
		AResource steel = new AResource(MAX);          
	...
	public void bankDrawResource(String resourceName, int amount) {
		//Method requires a correct string variable: "SOY", "HEMP", "PLASTIC", "GLASS", or "STEEL" and an int variable to control how many of said resource                                                             ////Method uses resourceList to call AResource.returnResource(amount) with correct Resource name
		int resourceNumber;
		switch(resourceName){
		 //Switch statment to control resource name: The number corresponding to the resources in the arrayList are mentioned above
		 case "SOY" :
				 resourceNumber = 0;
	...
	public void bankReturnResource(String resourceName,int amount) {
	 //Method requires a correct string variable: "SOY", "HEMP", "PLASTIC", "GLASS", or "STEEL" and an int variable to control how many of said resource
																  //Method uses resourceList to call AResource.returnResource(amount) with correct Resource name
		int resourceNumber;
		switch(resourceName) {
		//Switch statment to control resource name: The number corresponding to the resources int the arrayList are mentioned assigned above
	...
	public void printResourceList() {
		//method to print out resourceList
