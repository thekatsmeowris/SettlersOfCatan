...
public class Robber {
   ...
   */ 
    ...
    private void SevenResourceCheck()
    {
        ...
        {
            if(robberPlayers.get(i).countResources() >= 7) //If the current player has more than 7 resource cards...
            {
                System.out.println(robberPlayers.get(i).getNickname() + " lost:");
                for(int j = 0; j < (robberPlayers.get(i).countResources()/2); j++) //for loop to run half of number of player resource cards, rounding down. Ex: player has 7 cards it will run 3 times
                {
                    randomResourceNumber = RandomResourceValue(i);      //randomly generates a number from 0-4 using the RandomResourceValue method
                    robberPlayers.get(i).resources[randomResourceNumber]--;    //randomly removes one resource dictated by RandomResourceValue method  
